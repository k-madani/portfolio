...
export default function Navbar() {
  ...
  const handleNavClick = (e: React.MouseEvent<HTMLAnchorElement>, href: string) => {
    e.preventDefault();
    setMobileOpen(false);
    // Small delay so the menu closes before scroll starts (better UX)
    setTimeout(() => {
    ...
    <>
      ...
      >
        <div className="w-full max-w-7xl mx-auto flex items-center justify-between">
          ...
          {/* Mobile: theme toggle + hamburger — shown below md */}
          <div className="flex md:hidden items-center gap-2">
            ...
            >
              {mobileOpen ? (
                <X className="w-6 h-6 text-[rgb(var(--accent-primary))]" />
              ) : (
                <Menu className="w-6 h-6 text-[rgb(var(--accent-primary))]" />
              )}
            </button>
          </div>
        </div>
      ...
      <AnimatePresence>
        {mobileOpen && (
          <motion.div
            ...
            className="fixed inset-0 z-40 bg-[rgb(var(--bg-primary))]/95 backdrop-blur-lg md:hidden flex items-center justify-center"
          >
            <motion.div
              initial={{ opacity: 0, y: 20 }}
              animate={{ opacity: 1, y: 0 }}
            ...
            >
              {navItems.map((item, index) => (
                <motion.a
                  ...
                  onClick={(e) => handleNavClick(e, item.href)}
                  className="text-2xl font-medium tracking-widest text-[rgb(var(--text-primary))] hover:text-[rgb(var(--accent-primary))] transition-colors uppercase"
                >
                  {item.name}
                </motion.a>
              ))}
